...
guarded only two —  #43). It guards the subset that is **unambiguously**
...
def test_irreversible_and_guest_exec_cli_commands_double_confirm():
    ...
    # sets. VMware Knight marks 16 tools destructiveHint=True, 9 of which have a CLI
...
def test_double_confirm_prompts_twice(monkeypatch):
    ...
    from vmware_knight.cli import _common

    asked = []
    monkeypatch.setattr(typer, "confirm", lambda msg, **kw: asked.append(msg))
    _common._double_confirm("", "web-01")
