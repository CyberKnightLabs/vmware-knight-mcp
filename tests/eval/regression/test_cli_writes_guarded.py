...
the same enforcement @vmware_tool gives the MCP surface — so ``vmware-knight vm
...
The write set is DERIVED, never hand-listed ( #43): a tool annotated
...
CLI_DIR = _REPO / "vmware_knight" / "cli"
...
# every guarded VMware Knight CLI write mirrors exactly one MCP write tool.
...
def _mcp_tool_risk(tool: str, tool_module: dict[str, str]) -> object:
    ...
    mod = importlib.import_module(f"vmware_knight.mcp_server.tools.{tool_module[tool]}")
...
def test_guarded_cli_writes_carry_their_mcp_tool_name():
    ...
    for path in sorted(CLI_DIR.rglob("*.py")):
        if path.stem == "__init__":
            continue
        module = importlib.import_module(f"vmware_knight.cli.{path.stem}")
        tree = ast.parse(path.read_text(encoding="utf-8"))
        func_map, mods = _ops_refs(tree)
        for node in ast.walk(tree):
            if not isinstance(node, ast.FunctionDef):
