...
- R2: _safe_error call sites passed a constant 'knight' label instead of tool name
...
def test_safe_error_passes_domain_exception_messages() -> None:
    from vmware_knight.mcp_server.server import _safe_error
    ...
    from vmware_knight.ops.vm_lifecycle import TaskFailedError, VMNotFoundError
...
def test_safe_error_passes_connection_error_through() -> None:
    ...
    hint = "Connection to vcenter-prod dropped. Run 'vmware-knight doctor'."
...
def test_tool_errors_decorator_labels_with_function_name() -> None:
    ...
    """
    from vmware_knight.mcp_server._shared import tool_errors
    ...
    with patch("vmware_knight.mcp_server._shared._safe_error", side_effect=_probe_safe_error):
        @tool_errors("str")
    ...
    assert out == "Error: boom Run 'vmware-knight doctor' to verify connectivity and credentials."
...
def test_no_tool_module_passes_a_hardcoded_safe_error_label() -> None:
    ...
    tools_dir = _REPO / "vmware_knight" / "mcp_server" / "tools"
    offenders = []
...
def _ttl_entry(vm_name: str = "ttl-vm"):
    from vmware_knight.ops.ttl import TTLEntry
...
def test_ttl_entry_kept_on_transient_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    from vmware_knight.scanner import scheduler
...
def test_ttl_entry_removed_when_vm_already_gone(monkeypatch: pytest.MonkeyPatch) -> None:
    from vmware_knight.ops.vm_lifecycle import VMNotFoundError
    from vmware_knight.scanner import scheduler
...
def test_plan_create_vm_drops_none_optional_params(monkeypatch: pytest.MonkeyPatch) -> None:
    from vmware_knight.ops import plan_executor, vm_lifecycle
...
def test_find_triggered_alarm_destroys_container_exactly_once() -> None:
    ...
    from tests.eval.regression._pc_fakes import NoLazyMO, make_si
    from vmware_knight.ops.alarm_mgmt import _find_triggered_alarm
...
@pytest.mark.parametrize(
    "rel_path",
    [
        "vmware_knight/ops/guest_ops.py",
        "vmware_knight/ops/ova_deploy.py",
    ],
)
...
def test_cli_domain_exception_prints_teaching_error(
    ...
    from vmware_knight.cli import _common, app
    from vmware_knight.ops.vm_lifecycle import VMNotFoundError
    ...
    from vmware_knight.cli import vm as vm_module
