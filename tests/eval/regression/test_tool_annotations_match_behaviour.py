...
(CLAUDE.md ) retries transient failures once. A timeout on a snapshot
...
from vmware_knight.mcp_server import tools as mcp_server_tools
from vmware_knight.mcp_server.server import mcp
from vmware_knight.ops import guest_ops, vm_lifecycle
...
def test_the_read_write_marker_agrees_with_the_annotation() -> None:
    ...
    mechanical link, so either could drift alone ( #6). Checked over the
