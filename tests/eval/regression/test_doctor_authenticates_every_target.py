...
and left standing in the rest (CLAUDE.md  #7).
...
def _install(monkeypatch, config, failures=()):
    ...
    monkeypatch.setattr("vmware_knight.config.load_config", lambda: config)
    monkeypatch.setattr("vmware_knight.connection.ConnectionManager", _Mgr)
