...
and driving it, not from memory ( #36). ``mcp`` 1.28.1:
...
import vmware_knight.mcp_server.tools.cluster as cluster_tools
import vmware_knight.mcp_server.tools.deploy as deploy_tools
import vmware_knight.mcp_server.tools.plan as plan_tools
import vmware_knight.mcp_server.tools.vm as vm_tools
from vmware_knight.config import TargetConfig
from vmware_knight.mcp_server._shared import _DOCTOR_HINT
from vmware_knight.mcp_server.server import mcp

#: What a connection failure looks like on the way into a tool body.
DROPPED = "Connection to vcenter-prod dropped. Run 'vmware-knight doctor'."
...
def test_dict_shaped_success_is_not_flagged(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "vmware_knight.ops.cluster_mgmt.get_cluster_info",
...
def test_falsy_error_key_is_not_a_failure(monkeypatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "vmware_knight.mcp_server.tools.guest._get_connection", _connection_works
    )
    monkeypatch.setattr(
        "vmware_knight.mcp_server.tools.guest.guest_provision",
    ...
    monkeypatch.setattr(
        "vmware_knight.mcp_server.tools.guest.measure_guest_provision",
...
def test_failure_envelope_rule_matches_vmware_policys(payload) -> None:
    ...
    drifting ( #6). If the rule moves, it goes red here rather than silently
    producing frames that contradict the audit trail.
    """
    from vmware_policy.decorators import _returned_failure

    from vmware_knight.mcp_server._shared import _is_error_envelope
