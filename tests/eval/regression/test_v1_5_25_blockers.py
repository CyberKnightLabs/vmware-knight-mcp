...
def test_clone_vm_accepts_target_host_and_datastore() -> None:
    from vmware_knight.ops.vm_lifecycle import clone_vm
...
def test_clone_vm_passes_target_host_to_relocate_spec() -> None:
    ...
    """
    from vmware_knight.ops import vm_lifecycle
...
def test_migrate_vm_accepts_target_datastore() -> None:
    from vmware_knight.ops.vm_lifecycle import migrate_vm
...
def test_mcp_config_install_imports_json() -> None:
    ...
    import vmware_knight.cli.mcp_config as m
...
def test_wait_for_task_preserves_fault_cause() -> None:
    ...
    from vmware_knight.ops.vm_lifecycle import TaskFailedError, _wait_for_task
    ...
    with patch("vmware_knight.ops.vm_lifecycle.vim") as mock_vim:
...
def test_mcp_cmd_guards_python_version() -> None:
    ...
    import vmware_knight.cli._root as root
...
def test_mcp_server_uses_optional_not_pep604() -> None:
    ...
    Tool signatures now live in vmware_knight/mcp_server/tools/*.py plus the shared helpers in
    _shared.py; scan all of them, not just the (now thin) server.py entrypoint.
    """
    from pathlib import Path

    mcp_dir = Path(__file__).resolve().parents[3] / "vmware_knight" / "mcp_server"
    ...
    assert not bad, (
        "vmware_knight.mcp_server tool signatures must not use PEP 604 "
        "`X | None`:\n" + "\n".join(bad)
    )
