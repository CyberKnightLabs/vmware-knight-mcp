"""Regression — VMware Knight re-exposes cluster-health summary by delegating to monitor.

VMware Knight is the family's conversational entry point, so "what's wrong in my
environment?" must work from an VMware Knight conversation too. The aggregation lives in
vmware-monitor (a dependency); VMware Knight registers a thin ``cluster_health_summary``
MCP tool and ``summary`` CLI command that call it with VMware Knight's own connection —
...
def test_mcp_tool_registered_and_read_only() -> None:
    """The delegated summary tool is present on the VMware Knight MCP server."""
    from vmware_knight.mcp_server.server import mcp

    tools = {t.name for t in asyncio.run(mcp.list_tools())}
    assert "cluster_health_summary" in tools, "VMware Knight must re-expose cluster_health_summary"


def test_mcp_tool_delegates_to_monitor() -> None:
    """The tool calls the vmware-monitor aggregation with VMware Knight's connection."""
    from vmware_knight.mcp_server.tools import summary as tool_mod
...
def test_investigation_tools_registered() -> None:
    """VMware Knight re-exposes the object-investigation family + cross-vCenter attention."""
    from vmware_knight.mcp_server.server import mcp
    ...
    for name in (
        ...
        assert name in tools, f"VMware Knight must re-expose {name}"


def test_vm_bundle_delegates_to_monitor() -> None:
    """vm_investigation_bundle calls the monitor aggregation with VMware Knight's connection."""
    from vmware_knight.mcp_server.tools import summary as tool_mod
...
def test_cli_investigate_vm_delegates(tmp_path) -> None:
    """`vmware-knight investigate vm` renders a monitor bundle; --html writes offline."""
    bundle = {
    ...
    with (
        patch("vmware_knight.cli.investigate._get_connection", return_value=(object(), _Cfg())),
        patch(
            "vmware_monitor.ops.investigate_vm.get_vm_investigation_bundle", return_value=bundle
        ),
        patch("vmware_knight.cli.investigate._audit"),
    ):
...
def test_cli_attention_delegates(tmp_path) -> None:
    """`vmware-knight attention` connects to all targets and renders the merged view."""
    data = {
        "targets": [
            {
                "vcenter": "prod",
    ...
    with (
        patch(
            "vmware_knight.cli.investigate._get_all_connections",
...
def test_cli_summary_terminal_and_html(tmp_path) -> None:
    """`vmware-knight summary` renders via monitor; --html writes an offline file."""
    out = tmp_path / "snap.html"
    with (
        patch("vmware_knight.cli.summary._get_connection", return_value=(object(), _Cfg())),
        patch(
            "vmware_monitor.ops.cluster_summary.get_cluster_health_summary", return_value=_SAMPLE
        ),
        patch("vmware_knight.cli.summary._audit"),
    ):
