...
1. The gate inventory in ``capabilities.md`` is derived from the live registry
   ...
   and the code that  #6 says is the only thing that stops prose drifting.
...
CAPABILITIES = _REPO / "skills" / "vmware-knight" / "references" / "capabilities.md"
...
#: cheat-sheet entries are ("vmware-knight cluster delete ...  # double confirm").
...
_NAMES_A_SURFACE = re.compile(r"\bCLI\b|\bMCP\b|(?:^|\s)vmware-knight\s", re.IGNORECASE)
...
def test_no_document_promises_a_confirmation_without_naming_the_surface() -> None:
    ...
    # checking anything and would pass forever ( #1).
    assert matched >= 5, (
...
def test_a_confirm_gated_write_previews_and_then_writes(monkeypatch) -> None:
    ...
    from vmware_knight.ops import network_mgmt
    ...
    from vmware_knight.mcp_server.tools import network as _network_tools
    ...
    from vmware_knight.mcp_server.tools.network import create_dvs_portgroup
