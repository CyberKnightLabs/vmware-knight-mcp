...
This family ships a  2.0 baseline and Chinese documentation, so its users
...
Mechanical rather than remembered ( #6): a new undeclared read fails here.
...
PACKAGE = Path(__file__).resolve().parents[3] / "vmware_knight"


def test_every_text_read_declares_its_encoding() -> None:
    sources = sorted(PACKAGE.rglob("*.py"))
    assert sources, (
        f"no sources under {PACKAGE} — an empty scan is not a pass ( #1)"
    )
