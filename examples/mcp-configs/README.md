...
vmware-knight works with any MCP-compatible agent. For fully local operation (no cloud API):

```bash
# Example: Aider + Ollama + vmware-knight CLI
aider --conventions skills/vmware-knight/SKILL.md --model ollama/qwen2.5-coder:32b

# Example: Continue + Ollama + MCP Server
# Configure Continue with Ollama model + vmware-knight MCP server
```
