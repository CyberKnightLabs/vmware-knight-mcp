...
> **Start here**: install vmware-knight first, then add modules as needed.
> Run `vmware-knight hub status` to see which family members are installed.
...
- **The enforcement boundary is vCenter/ESXi RBAC**: an agent can do whatever the configured account can. Use a dedicated, least-privilege service account scoped to what the agent may change (a read-only role makes the skill read-only). Store its password in `~/.vmware-knight/.env` (0600) or a secret manager (`VMWARE_<TARGET>_PASSWORD`).
...
```bash
uv tool install git+https://github.com/CyberKnightLabs/vmware-knight-mcp.git@v1.12.10
...
vmware-knight is the entry point. Add modules for additional capabilities:
...
| VM lifecycle, deployment, guest ops | **vmware-knight** ← this skill |
...
| Compliance baselines (CIS /  / PCI-DSS), drift detection, LLM remediation advisor | **vmware-harden** (`uv tool install vmware-harden`) |
...
1. One glance --> `cluster_health_summary` (MCP) or `vmware-knight summary` (CLI). Read `top_issues` first — ranked anomalies (disconnected hosts, red/yellow alarms, capacity pressure), each with a drill-down hint; the per-cluster table is context
...
3. Save/share a snapshot --> `vmware-knight summary --html` writes an offline, timestamped HTML file (identical to `vmware-monitor summary --html` — same shared renderer)
4. **If vmware-monitor is not installed** --> this command/tool is unavailable (VMware Knight delegates to it); install `vmware-monitor`, or use the deeper per-object read tools in that skill

### Object-Centered Investigation → Act (drill-down before you change anything)

**Judgment**: after triage points at a problem object, drill in with one correlated read *before* actuating — the bundle aggregates the object with its surrounding infrastructure and recent history so you (and the operator) see the full picture, not a guess. VMware Knight is the conversational entry point, so triage → investigate → act stays in one conversation.
...
**Steps**:
1. `vmware-knight datastore browse <ds> --pattern "*.ova"` → confirm image present and size
2. `vmware-knight deploy ova <path> --name <date>-<owner>-<purpose> --datastore <ds>`
3. `vmware-knight vm guest-exec <name> --cmd /usr/bin/python3 --args "setup.py" --user admin` → if exit ≠ 0, **stop**, do not snapshot a half-provisioned VM
4. `vmware-knight vm snapshot-create <name> --name baseline` (only if multi-iteration testing; skip for one-shot)
5. `vmware-knight vm set-ttl <name> --minutes 480`
...
3. `vmware-knight vm migrate <name> --to-host <target>` — wait for task completion, do not assume success on return
...
```bash
# VM operations
vmware-knight vm power-on <name> [--target <t>]
vmware-knight vm power-off <name> [--force]
vmware-knight vm create <name> --cpu 4 --memory 8192 --disk 100
vmware-knight vm delete <name>
vmware-knight vm clone <name> --new-name <new> [--to-host <host>] [--to-datastore <ds>] [--power-on]
vmware-knight vm migrate <name> --to-host <host> [--to-datastore <ds>]
vmware-knight vm snapshot-create <name> --name <snap> [--description <text>] [--memory]
vmware-knight vm snapshot-list <name>
vmware-knight vm snapshot-revert <name> --name <snap>
vmware-knight vm snapshot-delete <name> --name <snap> [--remove-children] [--no-wait]
vmware-knight vm task-status <task-id>                      # poll an async (--no-wait) operation by id
vmware-knight vm set-ttl <name> --minutes 480 [--dry-run]   # double confirm; daemon auto-deletes VM on expiry

# Guest operations (requires VMware Tools)
vmware-knight vm guest-exec <name> --cmd <script-path> --args "<args>" --user <username>
vmware-knight vm guest-upload <name> --local ./script.sh --guest /tmp/script.sh --user <username>

# Deploy
vmware-knight deploy ova <path> --name <vm> --datastore <ds>
vmware-knight deploy linked-clone --source <vm> --snapshot <snap> --name <new>

# Cluster
vmware-knight cluster create <name> --ha --drs
vmware-knight cluster info <name>
vmware-knight cluster drs-rules <name>                                     # list DRS rules
vmware-knight cluster drs-rule-set <name> --rule <r> --enable|--disable [--dry-run]
vmware-knight cluster drs-rule-create <name> --rule <r> --type antiAffinity --vm <vm1> --vm <vm2> [--disabled] [--dry-run]
vmware-knight cluster drs-rule-delete <name> --rule <r> [--dry-run]        # VM-VM only; double confirm

# Datastore
vmware-knight datastore browse <ds> --pattern "*.ova"

# Alarm management
vmware-knight alarm list [--target <t>]
vmware-knight alarm acknowledge <entity_name> <alarm_name> [--target <t>]
vmware-knight alarm reset <entity_name> <alarm_name> [--target <t>]   # double confirm; see blast radius above

# Family
vmware-knight hub status        # show installed family members + install commands
```
