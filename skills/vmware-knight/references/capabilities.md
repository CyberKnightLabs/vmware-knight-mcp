...
[vmware-monitor](https://github.com/vmware-skills/VMware-Monitor) library using VMware Knight's own
...
Plans are stored in `~/.vmware-knight/plans/`, deleted on success, auto-cleaned after 24h.
...
| Audit Trail | Every MCP call and every CLI command that reaches vCenter logged to `~/.vmware/audit.db` (SQLite WAL, via vmware-policy; parameters, result, status, caller — credentials redacted). Most CLI writes also append to `~/.vmware-knight/audit.log`, with before/after state where the command captures it (power, delete, reconfigure, snapshot-revert, clone, migrate, clean-slate, cluster delete/configure, DRS rule delete) |
