...
from vmware_knight.ops.vm_lifecycle import _wait_for_task
...
def _require_cluster(
    ...
    if cluster is None:
        raise ClusterNotFoundError(
            ...
            f"(CLI: vmware-knight summary) to see every cluster name on this target, "
...
def create_cluster(
    ...
    if existing is not None:
        raise ClusterError(
            ...
            f"(CLI: vmware-knight cluster info '{cluster_name}') to inspect it, "
...
def delete_cluster(si: ServiceInstance, cluster_name: str) -> str:
    ...
    if cluster.host and len(cluster.host) > 0:
        ...
        raise ClusterError(
            ...
            f"(CLI: vmware-knight cluster remove-host), then retry the delete."
        )
