...
from vmware_knight.ops.guest_ops import _FAMILY_WINDOWS, _detect_shell
...
def _local_file(path: str) -> tuple[dict[str, Any], str | None]:
    ...
    if not local.is_file():
        return info, (
            ...
            "This path is read on the machine running vmware-knight, not inside the "
    ...
    if not os.access(local, os.R_OK):
        return info, (
            ...
            "permission to the user running vmware-knight, or pass a readable path."
        )
    try:
