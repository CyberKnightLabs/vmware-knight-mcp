...
from vmware_knight.connection import get_verify_ssl
...
def _require_vm_with_tools(
    ...
    if vm.runtime.powerState != vim.VirtualMachine.PowerState.poweredOn:
        raise GuestOpsError(
            ...
            f"run vm_power_on (CLI: vmware-knight vm power-on '{vm_name}'), wait for the "
...
def guest_upload(
    ...
    if not local.is_file():
        raise ValueError(
            ...
            f"This path is read on the machine running vmware-knight, not inside the "
    ...
    if not os.access(local, os.R_OK):
        raise PermissionError(
            ...
            f"(chmod +r) to the user running vmware-knight, or pass a readable "
            f"path to --local."
        )
