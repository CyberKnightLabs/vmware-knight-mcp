...
from vmware_knight.ops.planner import delete_plan, load_plan, save_plan
...
def _delete_acknowledged(si: ServiceInstance, vm_name: str, ack: Any) -> str:
    from vmware_knight.ops.vm_delete_gate import delete_vm_acknowledged
...
def _created_instance_uuid(si: ServiceInstance, vm_name: str) -> str | None:
    ...
    from vmware_knight.ops.inventory import find_vm_by_name
...
def _delete_created_vm(si: ServiceInstance, params: dict[str, Any]) -> str:
    ...
    from vmware_knight.ops.vm_lifecycle import _require_vm, _wait_for_task
...
def _dispatch(si: ServiceInstance, action: str, params: dict[str, Any]) -> str:
    ...
    if handler is None:
        raise ValueError(
            ...
            f"vm_apply_plan (CLI: vmware-knight plan list shows pending plans)."
        )
    result = handler()
...
def _dispatch_table(si: Any, params: dict[str, Any]) -> dict[str, Callable[[], Any]]:
    ...
    from vmware_knight.ops.vm_lifecycle import (
        clone_vm,
        create_snapshot,
        create_vm,
        delete_snapshot,
    ...
    from vmware_knight.ops.guest_ops import guest_download, guest_exec, guest_upload
    from vmware_knight.ops.vm_deploy import (
        attach_iso,
        convert_to_template,
...
def _cluster_create(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.cluster_mgmt import create_cluster
...
def _cluster_delete(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.cluster_mgmt import delete_cluster
    return delete_cluster(si, params["cluster_name"])


def _cluster_configure(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.cluster_mgmt import configure_cluster
...
def _cluster_add_host(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.cluster_mgmt import add_host_to_cluster
...
def _cluster_remove_host(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.cluster_mgmt import remove_host_from_cluster
...
def _iscsi_enable(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.iscsi_config import enable_software_iscsi
    return enable_software_iscsi(si, params["host_name"])


def _iscsi_add_target(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.iscsi_config import add_iscsi_target
...
def _iscsi_remove_target(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.iscsi_config import remove_iscsi_target
...
def _storage_rescan(si: ServiceInstance, params: dict[str, Any]) -> str:
    from vmware_knight.ops.iscsi_config import rescan_storage
