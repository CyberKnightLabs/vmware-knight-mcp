...
from vmware_knight.ops.inventory import (
    InventoryError,
...
# OVA import lives in its own module; re-exported so existing
# ``vm_deploy.deploy_ova`` call sites and ``batch_deploy`` keep working.
from vmware_knight.ops.ova_deploy import deploy_ova
...
_log = logging.getLogger("vmware-knight.deploy")
...
def load_deploy_spec(spec_path: str) -> dict:
    ...
    if not spec or "vms" not in spec:
        raise ValueError(
            ...
            f"in the vmware-knight repo — then re-run the batch deploy."
        )
