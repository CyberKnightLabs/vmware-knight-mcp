...
def resolve_datacenter(
    ...
    if datacenter_name:
        ...
        if dc is None:
            raise InventoryError(
                ...
                f"tree. Check you are on the right vCenter with 'vmware-knight doctor'."
            )
        return dc
    content = si.RetrieveContent()
    ...
    raise InventoryError(
        ...
        "--target points at the intended vCenter: run 'vmware-knight doctor'."
    )
    ...
    if cr is None:
        if cluster_name:
            raise InventoryError(
                ...
                f"'{dc.name}'. Run cluster_health_summary (CLI: vmware-knight summary) "
        ...
        raise InventoryError(
            ...
            f"place: run 'vmware-knight doctor'."
        )
    return cr
