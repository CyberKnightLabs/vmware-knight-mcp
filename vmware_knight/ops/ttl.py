...
Storage: ~/.vmware-knight/ttl.json  (JSON dict of vm_name → TTL entry)
"""
...
logger = logging.getLogger("vmware-knight.ttl")

_TTL_FILE = Path.home() / ".vmware-knight" / "ttl.json"
...
def _save_ttl_store(store: dict[str, dict]) -> None:
    ...
    from vmware_knight._fsutil import secure_chmod_file, secure_mkdir
...
def verify_ttl_identity(si: ServiceInstance, entry: TTLEntry) -> None:
    ...
    if entry.instance_uuid is None:
        return
    from vmware_knight.ops.vm_lifecycle import _require_vm
...
def measure_clean_slate(si: ServiceInstance, vm_name: str, snapshot_name: str) -> dict:
    ...
    from vmware_knight.ops.vm_lifecycle import _require_vm, matching_snapshots, snapshot_nodes
