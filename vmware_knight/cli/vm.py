...
def vm_power_on(
    ...
    """Power on a VM."""
    from vmware_knight.ops.vm_lifecycle import power_on_vm

    si, _ = _get_connection(target, config)
    if dry_run:
        from vmware_knight.ops.vm_lifecycle import get_vm_info
...
def vm_power_off(
    ...
    from vmware_knight.ops.vm_lifecycle import get_vm_info, power_off_vm
    ...
    if dry_run:
        ...
        return
    _show_state_preview(before, "", name)
    _double_confirm("", name, _resolve_target(target))
...
def vm_create(
    ...
    """Create a new VM."""
    from vmware_knight.ops.vm_lifecycle import create_vm
...
def vm_delete(
    ...
    from vmware_knight.ops.vm_delete_gate import vm_delete_blast_radius
    from vmware_knight.ops.vm_lifecycle import delete_vm, get_vm_info
...
def vm_reconfigure(
    ...
    from vmware_knight.ops.vm_lifecycle import get_vm_info, reconfigure_vm
    ...
    _double_confirm(f"({', '.join(changes)})", name, _resolve_target(target))
...
def vm_snapshot_create(
    ...
    from vmware_knight.ops.vm_lifecycle import create_snapshot
...
def vm_snapshot_list(
    ...
    """List VM snapshots."""
    from vmware_knight.ops.vm_lifecycle import list_snapshots
...
def vm_snapshot_revert(
    ...
    from vmware_knight.ops.vm_lifecycle import get_vm_info, revert_to_snapshot
    ...
    if dry_run:
        ...
        return
    _show_state_preview(before, "", vm_name)
    console.print(f"[bold yellow]  Snapshot: {snap_name}[/]")
    _double_confirm(f" '{snap_name}'", vm_name, _resolve_target(target))
...
def vm_snapshot_delete(
    ...
    from vmware_knight.ops.vm_lifecycle import delete_snapshot
    ...
    if dry_run:
        ...
        return
    si, _ = _get_connection(target, config)
    console.print(f"[bold yellow]⚠️   VM '{vm_name}'  '{snap_name}'[/]")
    _double_confirm(f" '{snap_name}'", vm_name, _resolve_target(target))
...
def vm_task_status(
    ...
    from vmware_knight.ops.vm_lifecycle import get_task_status
...
def vm_clone(
    ...
    from vmware_knight.ops.vm_lifecycle import clone_vm, get_vm_info
    ...
    _double_confirm(f" '{new_name}'", name, _resolve_target(target))
    result = clone_vm(
        si, name, new_name,
        target_host=to_host,
...
def vm_migrate(
    ...
    from vmware_knight.ops.vm_lifecycle import get_vm_info, migrate_vm
    ...
    if to_datastore:
        console.print(f"[bold yellow]  Target datastore: {to_datastore}[/]")
    _double_confirm(f" '{to_host}'", name, _resolve_target(target))
...
def vm_set_ttl(
    ...
    from vmware_knight.ops.ttl import preview_ttl, set_ttl
    ...
    if dry_run:
        ...
        return
    _double_confirm(f" TTL ({minutes} )", vm_name, _resolve_target(target))
...
def vm_cancel_ttl(vm_name: str) -> None:
    ...
    from vmware_knight.ops.ttl import cancel_ttl
...
def vm_list_ttl() -> None:
    ...
    from vmware_knight.ops.ttl import list_ttl
...
def vm_clean_slate(
    ...
    from vmware_knight.ops.vm_lifecycle import clean_slate, get_vm_info
    ...
    si, _ = _get_connection(target, config)
    before = get_vm_info(si, vm_name)
    _show_state_preview(before, "Clean Slate ()", vm_name)
    console.print(f"[bold yellow]  Snapshot: {snapshot}[/]")
    _double_confirm(f" '{snapshot}'", vm_name, _resolve_target(target))
...
def vm_guest_exec_cmd(
    ...
    from vmware_knight.ops.guest_ops import guest_exec
    ...
    _double_confirm(f"({command})", vm_name, _resolve_target(target))
    si, _ = _get_connection(target, config)
...
def vm_guest_upload_cmd(
    ...
    from vmware_knight.ops.guest_ops import guest_upload
    ...
    _double_confirm(f"({guest_path})", vm_name, _resolve_target(target))
    si, _ = _get_connection(target, config)
...
def vm_guest_download_cmd(
    ...
    from vmware_knight.ops.guest_ops import guest_download
