...
def _cli_error_types() -> tuple[type[BaseException], ...]:
    """Exception types translated to a one-line teaching error ( #37).
        ...
        try:
            ...
            if isinstance(e, KeyError):
                ...
                message = (
                    ...
                    "password in ~/.vmware-knight/.env (env var "
                    "VMWARE_<TARGET>_PASSWORD) and the username in "
                    "~/.vmware-knight/config.yaml. Re-run 'vmware-knight init' to reset both."
                )
                ...
                message = (
                    ...
                    "~/.vmware-knight/config.yaml (or re-run 'vmware-knight init')."
                )
...
def _get_connection(target: str | None, config_path: Path | None = None):
    ...
    from vmware_knight.config import load_config
...
def _double_confirm(
    ...
    """
    console.print(f"[bold yellow]⚠️  : {action} {resource_type} '{vm_name}'[/]")
    try:
        typer.confirm(f" 1 : {action} '{vm_name}'?", abort=True)
        typer.confirm(f" 2 : {action} '{vm_name}'，?", abort=True)
    except typer.Abort:
        _audit.log(
            target=target,
            operation=action,
