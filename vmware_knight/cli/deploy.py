...
def ds_browse(
    ...
    from vmware_knight.ops.datastore_browser import browse_datastore
...
def ds_scan_images(
    ...
    from vmware_knight.ops.datastore_browser import update_registry
...
def deploy_ova_cmd(
    ...
    from vmware_knight.ops.vm_deploy import deploy_ova
    ...
    if dry_run:
        ...
        return
    si, _ = _get_connection(target, config)
    _double_confirm(" OVA", name, _resolve_target(target))
    result = deploy_ova(
...
def deploy_template_cmd(
    ...
    from vmware_knight.ops.vm_deploy import deploy_from_template
    ...
    if dry_run:
        ...
        return
    si, _ = _get_connection(target, config)
    _double_confirm(f" '{template_name}' ", name, _resolve_target(target))
...
def deploy_linked_clone_cmd(
    ...
    from vmware_knight.ops.vm_deploy import linked_clone
    ...
    if dry_run:
        ...
        return
    si, _ = _get_connection(target, config)
    _double_confirm(f" '{source}@{snap}' ", name, _resolve_target(target))
    result = linked_clone(
...
def deploy_batch_cmd(
    ...
    from vmware_knight.ops.vm_deploy import batch_deploy, load_deploy_spec
    ...
    deploy_spec = load_deploy_spec(spec)
    vm_names = [v["name"] for v in deploy_spec["vms"]]
    console.print(f"[bold yellow] {len(vm_names)}  VM: {', '.join(vm_names)}[/]")
    _double_confirm(f" {len(vm_names)}  VM", ", ".join(vm_names), _resolve_target(target))
...
def deploy_batch_clone_cmd(
    ...
    from vmware_knight.ops.vm_deploy import batch_clone
    ...
    if dry_run:
        ...
        return
    si, _ = _get_connection(target, config)
    console.print(f"[bold yellow] {count} : {', '.join(vm_names)}[/]")
    _double_confirm(f" '{source}'  {count} ", source, _resolve_target(target))
    results = batch_clone(
...
def deploy_mark_template(
    ...
    from vmware_knight.ops.vm_deploy import convert_to_template
...
def deploy_iso_cmd(
    ...
    from vmware_knight.ops.vm_deploy import attach_iso
