...
def cluster_info_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import get_cluster_info
...
def cluster_delete_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import delete_cluster, get_cluster_info
    ...
    if dry_run:
        ...
        return
    _double_confirm("", name, _resolve_target(target), resource_type="Cluster")
...
def cluster_add_host_cmd(
    ...
    if dry_run:
        ...
        return
    si, _ = _get_connection(target, config)
    _double_confirm("", f"{host} → {name}", _resolve_target(target), resource_type="Host")
...
def cluster_configure_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import configure_cluster, get_cluster_info
...
def cluster_drs_rules_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import list_drs_rules
...
def cluster_drs_rule_set_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import set_drs_rule_enabled
    ...
    if dry_run:
        ...
        return
    _double_confirm(
        "/ DRS ", f"{rule_name} → {'enable' if enabled else 'disable'}",
...
def cluster_drs_rule_create_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import create_drs_rule
...
def cluster_drs_rule_delete_cmd(
    ...
    from vmware_knight.ops.cluster_mgmt import delete_drs_rule
    ...
    if dry_run:
        ...
        return
    _double_confirm(
        " DRS ", rule_name, _resolve_target(target), resource_type="DRS rule",
    )
