"""MCP server wrapping VMware Knight operations.
...
layer — each ``@mcp.tool()`` function (defined in ``vmware_knight/mcp_server/tools/``)
simply delegates to the corresponding function in the ``vmware_knight``
package.
...
Module layout
-------------
* ``vmware_knight/mcp_server/_shared.py`` — the shared ``mcp`` (FastMCP) instance, the
  connection helper, ``_safe_error``, and the ``@tool_errors`` decorator.
* ``vmware_knight/mcp_server/tools/*.py`` — one module per tool category; importing each
  registers its ``@mcp.tool()`` functions onto the shared ``mcp`` instance.
* this file — re-exports ``mcp`` and exposes ``main()`` (the ``vmware-knight-mcp``
  console script and the ``vmware-knight mcp`` subcommand both call it).
...
Source: https://github.com/CyberKnightLabs/vmware-knight-mcp
License: MIT
"""
...
from vmware_knight.mcp_server._shared import _safe_error, mcp, tool_errors

# Importing the tool modules registers every @mcp.tool() onto the shared
# `mcp` instance above. Order does not matter; each module is self-contained.
from vmware_knight.mcp_server.tools import (  # noqa: F401 — imported for registration side effects
    alarm,
    cluster,
    datastore,
    deploy,
...
from vmware_knight.policy_environment import _cached_config, _environment_for  # noqa: E402,F401
