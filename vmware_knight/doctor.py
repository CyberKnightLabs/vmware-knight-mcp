"""vmware-knight doctor — environment and connectivity diagnostics.
...
from vmware_knight.config import CONFIG_DIR, ENV_FILE, resolve_config_path
...
def _check_config_file() -> tuple[bool, str]:
    ...
    default path, because $VMWARE_KNIGHT_CONFIG moves the file the tools read
    and this doctor used to keep reporting on ~/.vmware-knight/config.yaml —
    ...
    return False, (
        f"Config not found: {path}  →  Run: vmware-knight init  "
...
def _check_env_file() -> tuple[bool, str]:
    if not ENV_FILE.exists():
        return False, (
            f".env not found: {ENV_FILE}  →  Run: vmware-knight init  "
...
def _check_auth() -> tuple[bool, str]:
    ...
    sibling skills already iterated; this one did not (CLAUDE.md  #7, a
...
def _check_mcp_server() -> tuple[bool, str]:
    ...
    try:
        ...
        importlib.import_module("vmware_knight.mcp_server.server")
