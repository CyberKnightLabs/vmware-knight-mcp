...
4. Generate plan with unique ID, write to `~/.vmware-knight/plans/{plan_id}.json`
...
Return all pending (not yet executed) plans from `~/.vmware-knight/plans/`.
...
| File | Change |
|------|--------|
| `vmware_knight/ops/planner.py` | **NEW** — plan CRUD, validation, pre-checks |
| `vmware_knight/ops/plan_executor.py` | **NEW** — sequential execution, rollback |
| `vmware_knight/mcp_server/server.py` | Add 3 MCP tools |
| `vmware_knight/cli.py` | Add `plan list` CLI command (read-only) |
