# Using vmware-knight with Goose

[Goose](https://github.com/block/goose) is an open-source AI agent by Block that runs locally on your machine. This guide shows how to add `vmware-knight` as an MCP extension so Goose can manage your VMware infrastructure using natural language.
...
2. **Configure credentials**
   ...
   cat > ~/.vmware-knight/config.yaml << 'EOF'
   targets:
     my-vcenter:
   ...
   echo "VMWARE_PASSWORD=your_password" > ~/.vmware-knight/.env
...
```bash
vmware-knight mcp-config install --agent goose
```
...
# Name: vmware-knight
# Command: vmware-knight mcp     # v1.5.15+; legacy: uvx --from vmware-knight vmware-knight-mcp
...
`vmware-knight` works with local models via Goose + Ollama. Recommended models for tool-calling:
...
extensions:
  vmware-knight:
    type: stdio
    cmd: uvx
    args: [--from, vmware-knight, vmware-knight-mcp]
    env:
      VMWARE_KNIGHT_CONFIG: ~/.vmware-knight/config.yaml
```
...
| Issue | Solution |
|-------|----------|
| Extension not found | Check that `vmware-knight mcp` runs successfully (v1.5.15+) or `which vmware-knight` returns a PATH |
| `invalid peer certificate: UnknownIssuer` (uvx) | Corporate TLS proxy not trusted by uv. Use `vmware-knight mcp` instead, or `export UV_NATIVE_TLS=true` |
| Auth failure | Run `vmware-knight doctor` to verify vCenter connectivity |
| Tool call timeout | Large inventories may take 10–30s; Goose default timeout may need increasing |
| `VMWARE_KNIGHT_CONFIG` not found | Use absolute path, not `~` expansion in config |
