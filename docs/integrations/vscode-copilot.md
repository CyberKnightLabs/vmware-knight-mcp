# Using vmware-knight with VS Code Copilot

VS Code's GitHub Copilot supports MCP servers via `.vscode/mcp.json`. This guide shows how to add `vmware-knight` so Copilot can manage your VMware infrastructure from within VS Code.
...
```json
{
  "servers": {
    "vmware-knight": {
      "type": "stdio",
      "command": "python",
      "args": ["-m", "vmware_knight.mcp_server"],
      "cwd": "/path/to/VMware Knight",
      "env": {
        "VMWARE_KNIGHT_CONFIG": "${env:HOME}/.vmware-knight/config.yaml"
      }
    }
  }
}
  ...
  "mcp": {
    "servers": {
      "vmware-knight": {
        ...
        "cwd": "/path/to/VMware Knight",
        "env": {
          "VMWARE_KNIGHT_CONFIG": "/Users/your-name/.vmware-knight/config.yaml"
        }
      }
    }
  }
...
```
You: @vmware-knight Show active alarms in prod-vcenter
...
```
You: @vmware-knight Deploy vm-test from template ubuntu-22-base,
...
```
You: @vmware-knight Run "systemctl status my-service" on vm-backend01
