# Using vmware-knight with Cursor

[Cursor](https://www.cursor.com) is an AI-powered code editor with native MCP support. This guide shows how to add `vmware-knight` as an MCP server so Cursor's AI can manage your VMware infrastructure.
...
```bash
vmware-knight mcp-config install --agent cursor
```
...
```bash
vmware-knight mcp-config generate --agent cursor
```
